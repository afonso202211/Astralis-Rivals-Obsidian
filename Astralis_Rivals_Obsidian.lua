-- Astralis [Rivals]
-- Complete Obsidian UI conversion of the supplied Rivals script.
-- Obsidian: https://github.com/deividcomsono/Obsidian

local function astralisShowLoadingNotification()
    local RunService = game:GetService("RunService")
    local guiParent = (gethui and gethui()) or game:GetService("CoreGui")
    local gui = Instance.new("ScreenGui")
    gui.Name = "AstralisRivalsLoadingNotification"
...
getgenv().AstralisModuleCache = getgenv().AstralisModuleCache or {}

local function astralisSafeRequire(moduleRef, timeoutSec)
    local cache = getgenv().AstralisModuleCache
    ...
    error("[Astralis [Rivals]] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end

getgenv().AstralisRequire = astralisSafeRequire

local function astralisPreloadCoreModules()
    ...
    pcall(function() astralisSafeRequire(modulesFolder:WaitForChild("EnumLibrary", 4)) end)
    pcall(function() astralisSafeRequire(modulesFolder:WaitForChild("Utility", 4)) end)
    local controllers = playerScripts:WaitForChild("Controllers", 8)
    pcall(function() astralisSafeRequire(controllers:WaitForChild("FighterController", 4)) end)
    pcall(function() astralisSafeRequire(controllers:WaitForChild("CameraController", 4)) end)
end

local function astralisHideErrorPromptsStep()
...
local function astralisRunLoadingBootstrap()
    local runService = game:GetService("RunService")
    local dismissLoading = astralisShowLoadingNotification()
    local hideErrors = true
    local hideConn = runService.RenderStepped:Connect(function()
        if hideErrors then astralisHideErrorPromptsStep() end
    end)

    task.wait(0.8)

    pcall(astralisPreloadCoreModules)
...
-- Astralis [Rivals] - Obsidian UI bootstrap
local OBSIDIAN_REPOSITORY = "https://raw.githubusercontent.com/deividcomsono/Obsidian/main/"

local function loadObsidianModule(path)
    local source = game:HttpGet(OBSIDIAN_REPOSITORY .. path)
    local chunk, loadError = loadstring(source)
    assert(chunk, ("[Astralis [Rivals]] failed to load Obsidian module %s: %s"):format(path, tostring(loadError)))
    return chunk()
end
...
local ASTRALIS_ACCENT = Color3.fromRGB(0, 200, 255)

-- Keep the original script's small HUD factory calls compatible while every
-- menu control itself is created by Obsidian.
...
ThemeManager:SetDefaultTheme({
    ...
    AccentColor = ASTRALIS_ACCENT,
    OutlineColor = Library.Scheme.OutlineColor,
    FontColor = Library.Scheme.FontColor,
    FontFace = "Code",
})

local function applyAstralisAccentTheme()
    if not Library or not Library.Scheme then return end

    Library.Scheme.AccentColor = ASTRALIS_ACCENT
...
applyAstralisAccentTheme()
getgenv().AstralisApplyAccentTheme = applyAstralisAccentTheme

local ASTRALIS_MENU_DISPLAY_ORDER = 2147483646
local ASTRALIS_COSMETIC_DISPLAY_ORDER = 2147483647
local ASTRALIS_KEYBIND_LIST_ORDER = 80
local ASTRALIS_GAMEPLAY_OVERLAY_ORDER = 10

local function applyAstralisUiLayering()
    pcall(function()
        if Library and Library.ScreenGui then
            Library.ScreenGui.DisplayOrder = ASTRALIS_MENU_DISPLAY_ORDER
            Library.ScreenGui.IgnoreGuiInset = true
            Library.ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        end
    end)
    pcall(function()
        local st = getgenv().AstralisCosmeticUIState
        if st and st.gui then
            st.gui.DisplayOrder = ASTRALIS_COSMETIC_DISPLAY_ORDER
            st.gui.IgnoreGuiInset = true
            st.gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        end
    end)
    pcall(function()
        local kb = getgenv().AstralisKeybindList
        if kb and kb.gui then
            kb.gui.DisplayOrder = ASTRALIS_KEYBIND_LIST_ORDER
            kb.gui.IgnoreGuiInset = true
            kb.gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        end
    end)
end
getgenv().AstralisApplyUiLayering = applyAstralisUiLayering

local Window = Library:CreateWindow({
    Title = 'Astralis [Rivals]',
    Footer = 'Astralis [Rivals]',
    Center = true,
    AutoShow = true,
    Resizable = true,
...
applyAstralisUiLayering()

getgenv().AstralisMenuCursor = getgenv().AstralisMenuCursor or {}

local function ensureAstralisMenuCursor()
    local mc = getgenv().AstralisMenuCursor
...
local function isAstralisMenuOpen()
    return Library ~= nil and not Library.Unloaded and Library.Toggled == true
end

local function shouldSuppressGameplayOverlays()
    return isAstralisMenuOpen()
end

ensureAstralisMenuCursor()

local MENU_CURSOR_HZ = 240
local MENU_CURSOR_BIND = "AstralisMenuCursorDraw"
local menuCursorAccum = 0

local function drawAstralisMenuCursor()
    local mc = getgenv().AstralisMenuCursor
    if not mc or not mc.cursor or not mc.outline then return end
    local open = isAstralisMenuOpen()
    if not open then
    ...
    local accent = (Library and Library.Scheme.AccentColor) or Color3.fromRGB(0, 200, 255)
...
pcall(function()
    RunService:BindToRenderStep(MENU_CURSOR_BIND, Enum.RenderPriority.Last.Value + 10, function(dt)
        ...
        drawAstralisMenuCursor()
    end)
end)

getgenv().AstralisIsMenuOpen = isAstralisMenuOpen

getgenv().AstralisLanguage = getgenv().AstralisLanguage or "english"

local AstralisWeaponTranslations = {
    korean = {
...
getgenv().AstralisTranslateWeapon = function(name)
    if not name or type(name) ~= "string" then return name end
    local lang = getgenv().AstralisLanguage or "english"
    if lang == "english" then return name end
    local pack = AstralisWeaponTranslations[lang]
    return (pack and pack[name]) or name
end

getgenv().AstralisTranslateLabel = function(text)
    if not text or type(text) ~= "string" then return text end
    local lang = getgenv().AstralisLanguage or "english"
    if lang == "english" then return text:lower() end
    local translated = getgenv().AstralisTranslateWeapon(text)
    if translated ~= text then return translated:lower() end
    return text:lower()
end

getgenv().AstralisGetUiFont = function()
    if getgenv().AstralisLanguage == "korean" and getgenv().AstralisUIFont then
        return getgenv().AstralisUIFont
    end
    if Library and Library.Scheme.Font then
        return Library.Scheme.Font
    end
    return getgenv().AstralisUIFont or Enum.Font.Roboto
end

getgenv().AstralisL = function(key, ...)
    local lang = getgenv().AstralisLanguage or "english"
    local pack = AstralisLocaleTable[lang] or AstralisLocaleTable.english
    local template = pack[key] or AstralisLocaleTable.english[key] or key
    if select("#", ...) > 0 then
...
local AstralisFontHttp = game:GetService("HttpService")

local AstralisFontSources = {
    english = { file = "astralis_rivals_ui_en.otf", fontFile = "astralis_rivals_ui_en.font", name = "AstralisRivalsUI_En", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
    korean = { file = "astralis_rivals_ui_ko.otf", fontFile = "astralis_rivals_ui_ko.font", name = "AstralisRivalsUI_Ko", url = "https://github.com/googlefonts/noto-cjk/raw/main/Sans/OTF/Korean/NotoSansCJKkr-Regular.otf" },
    spanish = { file = "astralis_rivals_ui_es.otf", fontFile = "astralis_rivals_ui_es.font", name = "AstralisRivalsUI_Es", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
}

local function loadAstralisLanguageFont(lang)
    lang = lang or getgenv().AstralisLanguage or "english"
    local src = AstralisFontSources[lang] or AstralisFontSources.english
    local ok, face = pcall(function()
        if isfile and writefile and getcustomasset then
            ...
            writefile(src.fontFile, AstralisFontHttp:JSONEncode(fontdata))
            return Font.new(getcustomasset(src.fontFile))
        end
        return nil
    end)
    if ok and face then
        getgenv().AstralisUIFont = face
        return face
    end
    getgenv().AstralisUIFont = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium)
    return getgenv().AstralisUIFont
end

getgenv().AstralisReloadLanguageFont = function(lang)
    lang = lang or getgenv().AstralisLanguage or "english"
    if lang == "english" and Library and Library.Scheme.Font then
        getgenv().AstralisUIFont = Library.Scheme.Font
        return Library.Scheme.Font
    end
    return loadAstralisLanguageFont(lang)
end
task.defer(function()
    pcall(function()
        getgenv().AstralisReloadLanguageFont(getgenv().AstralisLanguage)
    end)
end)

getgenv().AstralisDrawingTextPool = getgenv().AstralisDrawingTextPool or {}

getgenv().AstralisTrackDrawingText = function(obj)
    table.insert(getgenv().AstralisDrawingTextPool, obj)
    pcall(function()
        local face = getgenv().AstralisUIFont
        if face and obj.FontFace ~= nil then obj.FontFace = face end
    end)
    return obj
end

getgenv().AstralisApplyDrawingUIFont = function()
    local face = getgenv().AstralisUIFont
    if not face then return end
    for _, obj in ipairs(getgenv().AstralisDrawingTextPool) do
        pcall(function()
...
getgenv().AstralisApplyUiFont = function(obj, textSize)
    if not obj then return end
    if textSize then obj.TextSize = textSize end
    local font = getgenv().AstralisGetUiFont()
    pcall(function()
...
getgenv().AstralisApplyUIFont = function()
    local face = getgenv().AstralisGetUiFont()
    if not face then return end
    getgenv().AstralisUIFont = face
    ...
    if getgenv().AstralisApplyDrawingUIFont then pcall(getgenv().AstralisApplyDrawingUIFont) end
    if getgenv().AstralisApplyKbListTheme then pcall(getgenv().AstralisApplyKbListTheme) end
end

getgenv().AstralisHudFonts = {
    ...
    { n = "custom", draw = 2, face = function() return getgenv().AstralisUIFont or Font.fromEnum(Enum.Font.Gotham) end },
}
getgenv().AstralisHudFontNames = {}
for i, slot in ipairs(getgenv().AstralisHudFonts) do
    getgenv().AstralisHudFontNames[i] = slot.n
end
getgenv().AstralisHudFontIdx = 1
getgenv().AstralisHudDrawFont = 2

getgenv().AstralisHudFontIndexByName = function(name)
    for i, slot in ipairs(getgenv().AstralisHudFonts) do
...
getgenv().AstralisApplyHudFont = function(idx)
    idx = math.clamp(tonumber(idx) or getgenv().AstralisHudFontIdx or 1, 1, #getgenv().AstralisHudFonts)
    local slot = getgenv().AstralisHudFonts[idx]
    getgenv().AstralisHudFontIdx = idx
    getgenv().AstralisHudDrawFont = slot.draw or 2
    local face
    pcall(function() face = slot.face() end)
    if face then getgenv().AstralisEspUiFont = face end
    if _G.ESPObjects then
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
    ...
    for _, obj in ipairs(getgenv().AstralisDrawingTextPool or {}) do
        pcall(function()
            if obj.Font ~= nil then obj.Font = getgenv().AstralisHudDrawFont end
            if face and obj.FontFace ~= nil then obj.FontFace = face end
        end)
    end
end
...
local function alignAstralisMenuTabs()
    -- Obsidian manages its own sidebar/tab layout.
end

task.defer(alignAstralisMenuTabs)
...
local Utility           = astralisSafeRequire(ReplicatedStorage.Modules.Utility)
local EnumLibrary       = astralisSafeRequire(ReplicatedStorage.Modules.EnumLibrary)
...
getgenv().AstralisGetAspectStretch = getAspectStretch
getgenv().AstralisWorldToScreen = worldToScreen
getgenv().AstralisScreenAnchor = screenAnchor
getgenv().AstralisScreenCenter = screenCenter
getgenv().AstralisGetUnstretchedCameraCFrame = getUnstretchedCameraCFrame

_G.Features = _G.Features or {}
_G.Features.CrossbowSoundId = "rbxassetid://165946246"
getgenv().AstralisCombatLastShotAt = getgenv().AstralisCombatLastShotAt or 0

local function markCombatShot()
    getgenv().AstralisCombatLastShotAt = tick()
end
getgenv().AstralisMarkCombatShot = markCombatShot

getgenv().AstralisWorldToScreenEsp = function(worldPos, cam)
...
fovScreenGui.DisplayOrder    = ASTRALIS_GAMEPLAY_OVERLAY_ORDER
...
local function restoreGunSoundVolumes()
    local function tryRestore(root)
        ...
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("Sound") and isRivalsGunSound(d) and not d:GetAttribute("AstralisShootSound") then
                pcall(function()
                    if d.Volume <= 0 then
                        d.Volume = 1
                    end
...
local function isRivalsGunSound(sound)
    ...
    if sound:GetAttribute("AstralisShootSound") then
        return false
    end
...
local function bindGunSoundMuteGuard(sound)
    if typeof(sound) ~= "Instance" or not sound:IsA("Sound") or sound:GetAttribute("AstralisShootSound") then
        return
    end
    if not sound:GetAttribute("AstralisGunMuteBound") then
        sound:SetAttribute("AstralisGunMuteBound", true)
...
getgenv().AstralisRefreshGunSoundMute = refreshGunSoundMute
...
local function handleLocalShot(bypassCooldown)
    ...
    if muzzlePos then
        ...
        params.FilterDescendantsAstraliss = { LocalPlayer.Character }
        params.FilterType = Enum.RaycastFilterType.Blacklist
        local res = workspace:Raycast(muzzlePos, (endPos - muzzlePos).Unit * 1000, params)
        if res then
            endPos = res.Position
            hitAstralis = res.Instance
        end
    end

    if hitAstralis then
        notifyProjectileImpact(hitAstralis)
    end
    ...
    makebullettracer(muzzlePos, endPos, hitAstralis ~= nil)
end

getgenv().AstralisHandleLocalShot = handleLocalShot
        ...
        if myChar and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            if tr then
                ...
                if dist <= 1500 then
                    ...
                    local params = RaycastParams.new()
                    params.FilterDescendantsAstraliss = {myChar, char}
...
local AIMBOT_RENDER_BIND = "AstralisAimbotUpdate"
...
local fonts = {}
fonts.main = getgenv().AstralisUIFont or Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Black)
...
local function getActiveParticles()
    local count = 0
    local function countInAstralis(inst)
        for _, child in ipairs(inst:GetChildren()) do
            if child:IsA("ParticleEmitter") then
                count = count + 1
            end
            countInAstralis(child)
        end
    end
    countInAstralis(workspace.CurrentCamera)
    return count
end
...
hitSoundsTab:AddToggle("DisableGunSounds", {
    ...
    Callback = function(val)
        ...
        if val then
            if getgenv().AstralisRefreshGunSoundMute then
                getgenv().AstralisRefreshGunSoundMute()
            end
        else
...
local function clearVmPartHighlight(part)
    ...
    local legacy = part and part:FindFirstChild("AstralisVmPartHighlight")
...
local function syncVmPartHighlight(part, fillColor, outlineColor, fillTrans, outlineTrans)
    ...
    hl.Name = "AstralisVmPartHighlight"
...
local applyChams
local chamLoop
local CHAM_RENDER_BIND = "AstralisViewModelChams"
...
getgenv().AstralisUpdViewModelChams = updViewModelEffectLoop

applyChams = function(dt)
    if isAstralisMenuOpen and isAstralisMenuOpen() then
        return
    end
...
local function viewportloop()
    ...
    viewportLoop = game:GetService("RunService").RenderStepped:Connect(function()
        if isAstralisMenuOpen and isAstralisMenuOpen() then return end
...
local function bindViewModelAnimator(animator)
    ...
    if animator:GetAttribute("AstralisNoAnimBound") then
        return
    end
    animator:SetAttribute("AstralisNoAnimBound", true)
...
local function hookViewModelAnimations()
    ...
    if fp then
        ...
        if not vm:GetAttribute("AstralisNoAnimVmHooked") then
            vm:SetAttribute("AstralisNoAnimVmHooked", true)
...
local function stopNoAnimLoop()
    ...
    local vm = Workspace:FindFirstChild("ViewModels")
    if vm then
        vm:SetAttribute("AstralisNoAnimVmHooked", nil)
        for _, desc in ipairs(vm:GetDescendants()) do
            if desc:IsA("Animator") then
                desc:SetAttribute("AstralisNoAnimBound", nil)
            end
        end
    end
end
...
local modules = {
    enums = astralisSafeRequire(replicatedstorage.Modules.EnumLibrary),
    fighter = astralisSafeRequire(player.PlayerScripts.Controllers.FighterController),
    camcontrol = astralisSafeRequire(player.PlayerScripts.Controllers.CameraController),
    utility = astralisSafeRequire(replicatedstorage.Modules.Utility)
}

local MATCH_ID_ATTRS = {"MatchId", "DuelId", "RoundId", "GameId", "MatchUUID", "ArenaId", "AstralisId"}
...
local _astralisLobbyCache = { t = 0, v = true }
local function lobbyCache()
    local now = os.clock()
    if now - _astralisLobbyCache.t < 0.15 then
        return _astralisLobbyCache.v
    end
    _astralisLobbyCache.t = now
    _astralisLobbyCache.v = not liveMatch()
    return _astralisLobbyCache.v
end

getgenv().AstralisIsInActiveMatch = liveMatch
getgenv().AstralisIsInLobby = lobbyCache
...
local function getammo()
    ...
    local success, controller = pcall(function()
        return astralisSafeRequire(player.PlayerScripts.Controllers.FighterController)
    end)
    ...
    for _, targetplayer in players:GetPlayers() do
        if targetplayer ~= player and targetplayer.Character then
            ...
            if valid(char) then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local wts = getgenv().AstralisWorldToScreen or worldToScreen
...
local function stopsync()
    ...
    clrVoidSnap()
    config.voidspam.phase = nil
    if config.orbit.savedpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart and not getgenv().AstralisUndergroundEnabled then
...
local function startsync()
    ...
    config.orbit.connection = runsvc.Heartbeat:Connect(function(dt)
        ...
        if not getgenv().AstralisUndergroundEnabled then
            if localfighter and localfighter.Entity and localfighter.Entity.RootPart then
                local currentPos = localfighter.Entity.RootPart.CFrame
...
modules.camcontrol.Update = function(...)
    if getgenv().AstralisUndergroundEnabled then
        return oldcamupdate(...)
...
local function updatesling()
    ...
    if slingon then
        ...
        if getgenv().AstralisSetUnderground then
            getgenv().AstralisSetUnderground(true)
        end
        ...
        if not Toggles.AntiAimUnderground.Value then
            if getgenv().AstralisSetUnderground then
                getgenv().AstralisSetUnderground(false)
            end
        end
        vhState.active = false
    end
...
runsvc.RenderStepped:Connect(function()
    local inLobby = getgenv().AstralisIsInLobby and getgenv().AstralisIsInLobby()
...
local function mkStatusLbl()
    local label = getgenv().AstralisTrackDrawingText(Drawing.new("Text"))
    label.Size = 13
    label.Font = 2
    label.Outline = true
    label.Center = true
...
local function createHitNotifBox(text)
    ...
    local label = Library:CreateLabel({
        ...
        Font = Enum.Font.Code,
        TextSize = textSize,
        TextColor3 = hn.color or Library.Scheme.FontColor,
        TextTransparency = 1,
...
local function shouldNotifyPlayerHit(plr)
    ...
    local lastShot = getgenv().AstralisCombatLastShotAt or 0
...
local function pollHitNotifHealth()
    ...
    for _, plr in ipairs(players:GetPlayers()) do
        ...
        if cur < last - 0.01 then
            ...
            ragePerf.lastHitAtByPlayer[plr] = tick()
            if cur <= 0 then
                local tryKill = getgenv().AstralisTryKillSound
                if tryKill then
...
local function bindHitNotifCharacter(plr, char)
    ...
    hitNotifHumConn[plr] = hum.HealthChanged:Connect(function(newHp)
        ...
        if newHp <= 0 and localHit and localHit.plr and localHit.plr ~= player and not config.hitNotifications.enabled then
            local tryKill2 = getgenv().AstralisTryKillSound
            if tryKill2 then
...
if not config.profile.regionspoof then
    config.profile.regionspoof = { enabled = false, value = ".gg/getastralis" }
end
...
regionTab:AddInput("RegionSpoofValue", {
    Text = "region value",
    Default = ".gg/getastralis",
    Numeric = false,
    Finished = false,
    Placeholder = "type region...",
    Callback = function(val)
        config.profile.regionspoof.value = val or ".gg/getastralis"
...
v2:AddToggle("cframespf_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().AstralisConfigLoading then return end
    end
...
v2:AddToggle("cframefly_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().AstralisConfigLoading then
            return
        end
        if not state then
            cleanupfly()
...
getgenv().AstralisCleanupMovement = function()
    cleanupfly()
    _G.keyheldcframe = false
    _G.keyheldcframefly = false
    local char = LP.Character
...
local fighter_controller = astralisSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local local_fighter = fighter_controller.LocalFighter
local underground_enabled = false
local camera_controller = astralisSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
...
local function getFloorBelowPosition(pos)
    ...
    if local_fighter and local_fighter.Entity and local_fighter.Entity.RootPart then
        raycastParams.FilterDescendantsAstraliss = {local_fighter.Entity.RootPart.Parent}
    end
...
local function updantiaim(deltatime)
    ...
    if getgenv().AstralisConfigLoading then return end
    if getgenv().AstralisConfigLoading == nil then return end
...
getgenv().AstralisFlushMovementState = function()
    flushAntiAimMovementState()
    if getgenv().AstralisCleanupMovement then
        pcall(getgenv().AstralisCleanupMovement)
    else
        _G.keyheldcframe = false
        _G.keyheldcframefly = false
    end
...
getgenv().AstralisSyncAfterConfigLoad = function()
    if getgenv().AstralisFlushMovementState then
        pcall(getgenv().AstralisFlushMovementState)
    end
    ...
    if Toggles and Toggles.AntiAimUnderground then
        ...
        getgenv().AstralisUndergroundEnabled = underground_enabled
        if not underground_enabled then
            underground_oldpos = nil
        end
    else
        underground_enabled = false
        getgenv().AstralisUndergroundEnabled = false
        underground_oldpos = nil
    end
...
antiaimbox:AddToggle("AntiAimUnderground", {
    ...
    Callback = function(value)
        underground_enabled = value
            getgenv().AstralisUndergroundEnabled = value
            if getgenv().AstralisConfigLoading then return end
            if not value then
            underground_oldpos = nil
        end
...
getgenv().AstralisSetUnderground = function(val)
    underground_enabled = val
    getgenv().AstralisUndergroundEnabled = val
    if not val then
        underground_oldpos = nil
    end
end
...
local getAspectStretch = getgenv().AstralisGetAspectStretch
local worldToEspScreen = getgenv().AstralisWorldToScreenEsp or getgenv().AstralisWorldToScreen
local espScreenAnchor = getgenv().AstralisScreenAnchor
...
local ESP_RENDER_BIND = "AstralisESPUpdate"
...
local function trackUtilDrawingText(obj)
    if getgenv().AstralisTrackDrawingText then
        return getgenv().AstralisTrackDrawingText(obj)
    end
    return obj
end
...
local function makedrawtext(text, color)
    ...
    t.Center = true
    t.Font = getgenv().AstralisHudDrawFont or 2
    return t
end
            ...
            if onScreen and utilconfig.enable and isselected(realname) then
                ...
                if distdraw then
                    if utilconfig.distance then
                        ...
                        distdraw.Text = getgenv().AstralisL("studs_short", dist)
                        if showImg then
...
local function trackBloomEffect(bloom)
    if not bloom or not bloom:IsA("BloomEffect") or bloom.Name == "AstralisExtraBloom" then
        return
    end
...
local function applyBloomModifier()
    ...
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("BloomEffect") and child.Name ~= "AstralisExtraBloom" then
            child.Enabled = false
        end
    end

    if bloomCfg.Enabled then
        local extraBloom = Lighting:FindFirstChild("AstralisExtraBloom")
        if not extraBloom then
            extraBloom = Instance.new("BloomEffect")
            extraBloom.Name = "AstralisExtraBloom"
        ...
        local legacyExtra = Lighting:FindFirstChild("AstralisExtraBloom")
        if legacyExtra then
            legacyExtra:Destroy()
        end
        for _, child in ipairs(Lighting:GetChildren()) do
...
local function ensureExtraEffects()
    ...
    if ccCfg and ccCfg.Enabled then
        if not extraColorCorrection or not extraColorCorrection.Parent then
            ...
            extraColorCorrection.Name = "AstralisExtraColorCorrection"
...
local WATERMARK_TEXT = "Astralis [Rivals]"

local textChars = {}
for i = 1, #WATERMARK_TEXT do
    local char = getgenv().AstralisTrackDrawingText(Drawing.new("Text"))
    char.Size = 13
    char.Font = 2
    char.Outline = true
...
local ammoLabel = getgenv().AstralisTrackDrawingText(Drawing.new("Text"))
ammoLabel.Size = 13
ammoLabel.Font = 2
...
getgenv().AstralisPlayKillSound = playKillSound

getgenv().AstralisTryKillSound = function(plr, lastHp, newHp)
    if not ksCfg.Enabled then
        return
    end
    if not plr or plr == ksLocalPlayer then
...
local ASPECT_RATIO_BIND = "AstralisAspectRatio"
...
getgenv().AstralisStopSkyboxUpdater = stopSkyboxUpdater
...
game:GetService("ScriptContext").DescendantRemoving:Connect(function(descendant)
    if descendant == script then
        local stopSkyboxUpdater = getgenv().AstralisStopSkyboxUpdater
...
local WatermarkConnection = game:GetService('RunService').Heartbeat:Connect(function()
    ...
    if (tick() - FrameTimer) >= 1 then
        ...
        FrameTimer = tick()
        FrameCounter = 0
        Library:SetWatermark(('Astralis [Rivals] | %s fps'):format(
            math.floor(FPS)
        ))
    end
end)

Library:SetWatermark('Astralis [Rivals]')
...
Library:OnUnload(function()
    ...
    getgenv().AstralisKeybindList = nil
    Library.Unloaded = true
end)

Library.KeybindFrame.Visible = false

getgenv().AstralisKeybindList = getgenv().AstralisKeybindList or {}
local kbList = getgenv().AstralisKeybindList
local KB_ROW_H = 18
local KB_PAD = 8
local KB_TITLE_H = 28
...
local function resolveKbFeatureName(idx, opt)
    local mapped = ASTRALIS_KB_LABELS[idx]
...
local function kbTextWidth(text, textSize)
    ...
    local ok, sz = pcall(function()
        local fontFace = getgenv().AstralisGetUiFont and getgenv().AstralisGetUiFont()
...
local function buildKbListUi()
    ...
    local gui = Instance.new("ScreenGui")
    gui.Name = "AstralisRivalsKeybindList"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    gui.DisplayOrder = ASTRALIS_KEYBIND_LIST_ORDER
    gui.Enabled = false
    gui.Parent = parent
    ...
    getgenv().AstralisApplyUiFont(title, 14)
    ...
    for i = 1, KB_MAX_ROWS do
        ...
        lbl.Parent = rowFrame
        getgenv().AstralisApplyUiFont(lbl, 12)
...
applyKbListTheme = function()
    ...
    if kbList.title then
        ...
        getgenv().AstralisApplyUiFont(kbList.title, 14)
    end
    ...
    for _, row in ipairs(kbList.rowPool) do
        if row.lbl then
            ...
            getgenv().AstralisApplyUiFont(row.lbl, 12)
        end
        if row.frame then
...
getgenv().AstralisApplyKbListTheme = function()
    kbThemeDirty = true
    applyKbListTheme()
end
getgenv().AstralisRefreshKbList = function(force)
...
do
    local prevSync = getgenv().AstralisSyncAfterConfigLoad
    getgenv().AstralisSyncAfterConfigLoad = function()
...
local function syncMenuOverlayUi()
    ...
    if open and getgenv().AstralisCosmeticUIState and getgenv().AstralisCosmeticUIState.applyLiveViewModelUpdate then
        task.defer(function()
            pcall(getgenv().AstralisCosmeticUIState.applyLiveViewModelUpdate)
        end)
    end
    if open and getgenv().AstralisApplyCosmeticUiTheme then
        pcall(getgenv().AstralisApplyCosmeticUiTheme)
    end
end

local lastAstralisMenuState = nil
Library:GiveSignal(RunService.RenderStepped:Connect(function()
    local open = isAstralisMenuOpen()
    if open ~= lastAstralisMenuState then
        lastAstralisMenuState = open
        syncMenuOverlayUi()
    end
end))
...
MenuGroup:AddDropdown('AstralisLanguage', {
    Text = 'language',
    Values = { 'english', 'korean', 'spanish' },
    Default = getgenv().AstralisLanguage or 'english',
    Callback = function(val)
        getgenv().AstralisLanguage = val
        if getgenv().AstralisReloadLanguageFont then
            getgenv().AstralisReloadLanguageFont(val)
        end
        if getgenv().AstralisApplyUIFont then
            getgenv().AstralisApplyUIFont()
        end
        if getgenv().AstralisRefreshLocaleUI then
            pcall(getgenv().AstralisRefreshLocaleUI)
        end
    end,
})
...
(function()
    ...
    local function isRigAstralis(obj)
    ...
    local function getServerRigs()
        ...
        if liveFolder and typeof(liveFolder) == "Instance" then
            ...
            local function tryAdd(rig)
                if isRigAstralis(rig) and rig.Parent and not added[rig] then
                    added[rig] = true
                    table.insert(serverRigCache, rig)
                end
            end
            if isRigAstralis(liveFolder) then
                tryAdd(liveFolder)
            end
            pcall(function()
                for _, child in liveFolder:GetChildren() do
                    if isRigAstralis(child) and (child:FindFirstChild("HumanoidRootPart") or child:FindFirstChild("RootPart")) then
                        tryAdd(child)
                    end
                end
            end)
            if #serverRigCache == 0 and isRigAstralis(liveFolder) then
                tryAdd(liveFolder)
            end
        end

        local entity = getFighterEntity()
        if isRigAstralis(entity) then
            local dup = false
...
do
    local oldBlur = Lighting:FindFirstChild("AstralisTextureBlur")
    if oldBlur then
        oldBlur:Destroy()
    end
end
...
local function astralisHasTargetTexture(inst)
...
local function recordTargetTexture(inst)
    if not inst or targetTextureOrig[inst] or not astralisHasTargetTexture(inst) then return end
...
local function degradeTextureAstralis(inst)
    if not inst or not astralisHasTargetTexture(inst) then return end
...
local function processTargetTextureAstralis(inst)
    if not inst then return end
    if astralisHasTargetTexture(inst) then degradeTextureAstralis(inst) end
    for _, child in ipairs(inst:GetDescendants()) do
        if astralisHasTargetTexture(child) then degradeTextureAstralis(child) end
    end
end
...
local function startTargetTextureFilter()
    ...
    targetTextureConn = game.DescendantAdded:Connect(function(inst)
        task.wait(0.1)
        processTargetTextureAstralis(inst)
    end)
end
...
getgenv().AstralisUpdSmoothTextures = updSmoothTextures
getgenv().AstralisUpdDarkTextures = updDarkTextures
getgenv().AstralisUpdTransparentTextures = updTransparentTextures
...
texturesBox:AddToggle('smooth_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().AstralisUpdSmoothTextures
        if fn then fn() end
    end
})
...
texturesBox:AddToggle('dark_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().AstralisUpdDarkTextures
        if fn then fn() end
    end
})
...
local transparentToggle = texturesBox:AddToggle('transparent_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().AstralisUpdTransparentTextures
        if fn then fn() end
    end
})
...
SaveManager:SetFolder('Astralis/Rivals')

local themeManagerThemeUpdate = ThemeManager.ThemeUpdate
function ThemeManager:ThemeUpdate()
    themeManagerThemeUpdate(self)
    if getgenv().AstralisApplyAccentTheme then
        pcall(getgenv().AstralisApplyAccentTheme)
    end
    if applyTargetHudTheme then
        applyTargetHudTheme()
    end
    if getgenv().AstralisApplyCosmeticUiTheme then
        pcall(getgenv().AstralisApplyCosmeticUiTheme)
    end
    if getgenv().AstralisKeybindList and getgenv().AstralisKeybindList.inner then
        if getgenv().AstralisApplyKbListTheme then
            pcall(getgenv().AstralisApplyKbListTheme)
        end
    end
end
...
getgenv().AstralisConfigLoading = true
applyAstralisAccentTheme()

task.defer(function()
    task.wait(0.35)
    pcall(applyAstralisAccentTheme)
    pcall(function()
    ...
    task.wait(1.25)
    pcall(applyAstralisAccentTheme)
    getgenv().AstralisConfigLoading = false
end)
end)()
